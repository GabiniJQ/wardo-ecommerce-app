...
import { z } from 'zod'
import citiesAndDepartments from '@/shared/utils/citiesAndDepartments.json'
...
const AddressForm = ({ addressData }: AddressFormProps) => {
  ...
  const defaultValues: AddressFormData = addressData
    ? {
        ...
        addressType: allowedTypes.includes(
  ...
  const onSubmit = (values: AddressFormData) => {
    if (!user?._id) return
    const { fullName, addressType, addressInfo, city, department, phone } =
      values
  ...
  useEffect(() => {
    ...
    if (departmentValue) {
      const cities = citiesAndDepartments[
        departmentValue as keyof typeof citiesAndDepartments
      ] as string[]
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <div className='flex'>
                    <div className='flex items-center leading-0 justify-center border-input border border-r-gray-100 rounded rounded-r-none p-1'>
                      +57
                    </div>
                    <FormControl>
                      <Input
